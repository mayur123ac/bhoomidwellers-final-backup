...
//
// Security: the session role is read from the HMAC-signed session cookie.
...
export async function PUT(req: NextRequest) {
  ...
  for (const u of updates) {
    if (!VALID_SCOPES.has(u.scope)) {
      return NextResponse.json(
        {
          success: false,
          message: `Invalid scope "${u.scope}". Must be CP_ENQUIRY, CP_LINKED_LEAD, or LEAD_PHONE.`,
        },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    // Return the current full policy state so the frontend can update in place.
    const rows = await getAllPoliciesForOrg(orgId);
    const policyMap: Record<string, Record<string, boolean>> = {};
    for (const scope of ["CP_ENQUIRY", "CP_LINKED_LEAD", "LEAD_PHONE"] as PhoneScope[]) {
      policyMap[scope] = {};
