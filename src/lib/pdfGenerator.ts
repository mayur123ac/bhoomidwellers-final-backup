...
export async function generatePdfBuffer(booking: any, lead: any, images: Record<string, string | null>) {
  try {
    ...
    const parseVal = (val: any) => {
      if (!val) return 0;
      let str = String(val).toLowerCase().replace(/[â‚¹\s,]/g, "");
    ...
    for (let i = 0; i < jointApplicants.length; i++) {
      ...
      const jaPanBase64 = images[`joint_${i}_pan`] || null;
      const jaAadhaarBase64 = images[`joint_${i}_aadhaar_front`] || null;
    ...
    <body>
      ...
      <div class="grid-container property-grid">
        ...
        <div class="field-row"><div class="field-label">Consideration Value</div><div class="field-colon">:</div><div class="field-val">â‚¹ ${booking.consideration_value || ''}</div></div>
      ...
      <div class="grid-container">
        ...
        <div class="checkbox-row">
          <div>â˜‘ Declaration Accepted</div>
          <div>â˜‘ Terms Accepted</div>
          <div>â˜‘ Consent Accepted</div>
        </div>
      </div>
    ...
    // Force turbopack to rebuild this file
