...
export default function AdminAssistantDock({ theme, isDark }: Props) {
    ...
    if (!open) {
        return <AdminAssistantLauncher isDark={isDark} onOpen={() => setOpen(true)} />;
    }
    const shell = isDark
    ...
    return (
        <>
            ...
            {/* <div aria-hidden="true" className="bdai-halo fixed inset-x-3 bottom-3 z-40 sm:inset-x-auto sm:bottom-6 sm:right-6 sm:w-[420px]"
...
@media (prefers-reduced-motion: reduce){
  .bdai-panel{ animation:none; }
  .bdai-send:not(:disabled):hover{ transform:none; }
}

@keyframes bdai-panel-in{
